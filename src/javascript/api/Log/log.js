(function () {
    ...
    function Log(options) {
        var __self = this,
            ...
            __maxLogCont = (options && typeof options.maxCount === "number" && options.maxCount>0) ? options.maxCount : null,
            __maxLogLength = (options && typeof options.maxLength === "number" && options.maxLength>0) ? options.maxLength : null,
            __currentLogLength = 0,
        ...
        var __init = function () {
            ...
            __log = function (params,arg) {
                ...
                if(__save && arg) {
                    ...
                    __currentLogLength += logStr.length;
                    ...
                    if(__maxLogCont && __logs.length>__maxLogCont) {
                        var tmpLogs = __logs.splice(__logs.length - __maxLogCont, __logs.length);

                        var tmpLength = 0;

                        for (var i = 0; i < __logs.length; i++) {
                            tmpLength += __logs[i].log.length;
                        }

                        __currentLogLength -= tmpLength;
                        __logs = tmpLogs;
                    }


                    if(__maxLogLength && __currentLogLength>__maxLogLength) {
                        var tmpNeedLen = __currentLogLength - __maxLogLength;// len need remove
                        var tmpLength = 0;
                        var selectedIndex = -1;
                        ...
                        if(selectedIndex !== -1) {
                            __currentLogLength -= tmpLength;
                            __logs = __logs.splice(selectedIndex+1, __logs.length);
                        }
                    }
