...
(function ($) {
    ...
    function winnerBubbles(match) {
        ...
        winner.append('<div class="bubble" >'+label.first+'</div>');
        var loser = el.find('.team.lose');
        loser.append('<div class="bubble">'+label.second+'</div>');
    ...
    function connector(height, shift, teamCon, align) {
        ...
        var width = parseInt($('.round:first').css('margin-' + align), 10) / 2;
        var drop = true;
        // drop:
        // [team]'\
        //         \_[team]
    ...
    var JqueryBracket = function (opts) {
        ...
        function mkMatch(round, data, idx, results, renderCb) {
            ...
            function teamElement(round, team, isReady) {
                ...
                var sEl = $('<div class="" data-resultid="result-' + rId + '"></div>');
                var score = "__";
                ...
                var tEl = $('<div class="team" style="height: 30px"></div>');
                var nEl = $('<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 bracketLabel" style="width: 100%;height: 100%"></div>').appendTo(tEl);
                if (round === 0)
                    ...
                    nEl.click(function () {
                        ...
                        function editor() {
                            function done_fn(val, next) {
                                ...
                                renderAll(true);
                                span.click(editor);
                                var labels = opts.el.find('.team[data-teamid=' + (team.idx + 1) + '] div.bracketLabel:first');
        ...
        var height = data.teams.length * 80;
    ...
    var methods = {
        init: function (opts) {
            ...
            if(opts.label) {
                if(opts.label.first) {
                    label.first = opts.label.first;
                }
                if(opts.label.second) {
                    label.second = opts.label.second;
                }
            }
            opts.el = this;
